    ...
    return(
        <div className="contenedor-principal-create">
            ...
            <form  id='form' name='form' onSubmit ={(e) => handleSubmit(e)}>
                ...
                <div className="label"> 
                    <label>Altura: </label>
                    <input type='number' value={input.min_Height} name='min_Height' min='10' max='80' onChange={(e)=>handleChange(e)} placeholder='Menor altura' required/>cm 
                    <input type='number' value={input.max_Height} name='max_Height' min='15' max='100' onChange={(e)=>handleChange(e)} placeholder='Mayor altura' required/>cm {errors.max_Height && (<p>{errors.max_Height}</p>)}
                    
                </div>
                <div className="label">
                    <label>Peso: </label>
                    <input type='number' value={input.min_Weight} name='min_Weight' min='1' max='90' onChange={(e)=>handleChange(e)} placeholder='Menor peso' required/>Kg 
                    <input type='number' value={input.max_Weight} name='max_Weight' min='2' max='100' onChange={(e)=>handleChange(e)} placeholder='Mayor peso' required/>Kg {errors.max_Weight && (<p>{errors.max_Weight}</p>)}         
                </div>
                <div className="label">
                    <label>Espectativa de vida: </label>
                    <input type='number' value={input.min_Year} name='min_Year' min='1' max='5' onChange={(e)=>handleChange(e)} placeholder='Menor edad'required/>años 
                    <input type='number' value={input.max_Year} name='max_Year' min='2' max='20' onChange={(e)=>handleChange(e)} placeholder='Mayor edad' required/>años {errors.max_Year && (<p>{errors.max_Year}</p>)}
                </div>

                <div className="label">
                    <label>Imagen: </label>
                    <input type='text' alt='not found' value={input.image} name='image' pattern='https?:\/\/[\w\-]+(\.[\w\-]+)+[/#?]?.*$' title='FORMATO URL'onChange={(e)=>handleChange(e)} placeholder='URL de imagen' required/>
                </div>
                <div className="label">
            ...
            <div>
                {[
                    input.temperament.map((e) =><div className='label'><p key={e.id}> {temperaments.find((v) => v.id === e)?.name} </p> <button onClick={()=> handleDelete(e)}>X</button></div> )   
                ]}
            </div>
            </div>        
        </div>
